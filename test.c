...
int main(){
	wprintf(L"Testing ComputeScore_CStrings...\n%d\n", ComputeScore_CStrings("Jupiter", "Jupiter"));
	wprintf(L"Testing ComputeScore_WCStrings...\n%d\n", ComputeScore_WCStrings(L"Jupiter", L"Jupiter"));
	...
	Match_ArraysOfWCStrings(NUM_STRINGS, a, NUM_STRINGS, b, results, results_scores);
	wprintf(L"+-----------------------+-----------------------+-------+\n");
	...
	wprintf(L"+-----------------------+-----------------------+-------+\n");
	{
		...
		for(i = 0; i < NUM_STRINGS; ++i){
			wprintf(L"| %-21ls | %-21ls | %5d |\n", a[i], b[results[i]], results_scores[i]);
		}
	}
	wprintf(L"+-----------------------+-----------------------+-------+\n");
	return 0;
